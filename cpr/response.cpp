...
Response::Response(std::shared_ptr<CurlHolder> curl, std::string&& p_text, std::string&& p_header_string, Cookies&& p_cookies = Cookies{}, Error&& p_error = Error{}) : text(std::move(p_text)), cookies(std::move(p_cookies)), error(std::move(p_error)), raw_header(std::move(p_header_string)) {
    ...
    curl_easy_getinfo(curl->handle, CURLINFO_TOTAL_TIME, &elapsed);
    char* url_string{nullptr};
    curl_easy_getinfo(curl->handle, CURLINFO_EFFECTIVE_URL, &url_string);
    url = Url(url_string);
#if LIBCURL_VERSION_NUM >= 0x073700 // 7.55.0 
    curl_easy_getinfo(curl->handle, CURLINFO_SIZE_DOWNLOAD_T, &downloaded_bytes);
    curl_easy_getinfo(curl->handle, CURLINFO_SIZE_UPLOAD_T, &uploaded_bytes);
#else
...
#endif
    curl_easy_getinfo(curl->handle, CURLINFO_REDIRECT_COUNT, &redirect_count);
}
